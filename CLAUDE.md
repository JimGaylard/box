...
- **Always Apply Changes**: After making any change in this repo (dotfiles, playbooks, packages), always apply it by running the appropriate `bin/install.sh` invocation before considering the task done. Use `--tags dotfiles` for dotfile-only edits; run the full playbook when packages or setup modules change. Never leave changes unapplied.
